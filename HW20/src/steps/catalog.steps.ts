...
Then('the catalog page displays {int} books', async function (this: AtenaBooksWorld, count: number) {
    await expect(this.catalogPage.books).toHaveCount(count);
});

When('the user remembers the data of the first book', async function (this: AtenaBooksWorld) {
    this.rememberedBook = await this.catalogPage.getDataOfBook(1);
});

When('the user opens the first book', async function (this: AtenaBooksWorld) {
    await this.catalogPage.openBook(1);
});

Then('the title, author and price on the book page match the remembered data', async function (this: AtenaBooksWorld) {
